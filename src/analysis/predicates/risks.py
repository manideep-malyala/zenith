...
class RiskPredicates:
    ...
    def __init__(self, core, idioms, composition, inheritance, coupling, graph_api, oop):
        self.core = core
        self.idioms = idioms
    ...
    def _risk_result(self, class_fqn: str, risk_name: str, confidence: str, matched: bool, signals: list, evidence: set[str], explanation: str) -> PredicateResult:
    ...
    def method_count(self, class_fqn: str) -> tuple[int, set[str]]:
        res = self.graph_api._get_outgoing_edges(class_fqn, EdgeType.CONTAINS)
        return len(res.nodes), res.get_evidence_fact_ids()

    def fan_out(self, class_fqn: str) -> tuple[int, set[str]]:
        comp = self.graph_api._get_outgoing_edges(class_fqn, EdgeType.COMPOSES)
        inst = self.graph_api._get_outgoing_edges(class_fqn, EdgeType.INSTANTIATES)
        ...
        methods = self.graph_api._get_outgoing_edges(class_fqn, EdgeType.CONTAINS)
        for m in methods.nodes:
            calls = self.graph_api._get_outgoing_edges(m, EdgeType.CALLS)
    ...
    def fan_in(self, class_fqn: str) -> tuple[int, set[str]]:
        comp_in = self.graph_api._get_incoming_edges(class_fqn, EdgeType.COMPOSES)
        inst_in = self.graph_api._get_incoming_edges(class_fqn, EdgeType.INSTANTIATES)
    ...
    def dependency_diversity(self, class_fqn: str) -> tuple[int, set[str]]:
        ...
        return count, evidence # Proxy for diversity for now
    ...
    def class_size(self, class_fqn: str) -> tuple[int, set[str]]:
        ...
        return mc, ev_mc # Proxy class size by method count

    def inheritance_depth(self, class_fqn: str) -> tuple[int, set[str]]:
        if not self.graph_api or class_fqn not in self.graph_api.graph:
            return 0, set()
        # Find shortest path from root 'object' or just max path in ancestors
        try:
    ...
    def responsibility_diversity(self, class_fqn: str) -> tuple[int, set[str]]:
        ...
        db = self.core.uses_sqlite3(class_fqn) # Proxy for DB
    ...
    def has_srp_risk(self, class_fqn: str) -> PredicateResult:
        ...
        return self._risk_result(class_fqn, "SRP Risk", "MEDIUM" if matched else "LOW", matched, ["srp_risk"],
    ...
    def has_ocp_risk(self, class_fqn: str) -> PredicateResult:
        ...
        return self._risk_result(class_fqn, "OCP Risk", "MEDIUM" if matched else "LOW", matched, ["ocp_risk"],
    ...
    def has_lsp_risk(self, class_fqn: str) -> PredicateResult:
        ...
        return self._risk_result(class_fqn, "LSP Risk", "LOW", matched, ["lsp_risk"],
    ...
    def has_isp_risk(self, class_fqn: str) -> PredicateResult:
        ...
        return self._risk_result(class_fqn, "ISP Risk", "MEDIUM" if matched else "LOW", matched, ["isp_risk"],
    ...
    def has_dip_risk(self, class_fqn: str) -> PredicateResult:
        fout, ev_fout = self.fan_out(class_fqn)
        # Check if it depends on abstractions (heuristically, do its dependencies have subclasses?)
        comp = self.graph_api._get_outgoing_edges(class_fqn, EdgeType.COMPOSES)
        concrete_deps = 0
        for c in comp.nodes:
        ...
        return self._risk_result(class_fqn, "DIP Risk", "MEDIUM" if matched else "LOW", matched, ["dip_risk"],
    ...
    def has_god_class(self, class_fqn: str) -> PredicateResult:
        ...
        return self._risk_result(class_fqn, "God Class", "HIGH" if matched else "LOW", matched, ["god_class"],
    ...
    def has_god_method(self, class_fqn: str) -> PredicateResult:
        # Cannot easily check method size without line numbers in fact base, but we can check branch complexity of class
        bc, ev_bc = self.branch_complexity(class_fqn)
        matched = bc >= 2 # crude proxy
        return self._risk_result(class_fqn, "God Method / Complexity", "LOW", matched, ["complex_method"],
                                 ev_bc, f"Class exhibits high branch complexity ({bc}).")

    def has_feature_envy(self, class_fqn: str) -> PredicateResult:
        # A class that calls many methods on a single composed object
        comp = self.graph_api._get_outgoing_edges(class_fqn, EdgeType.COMPOSES)
        delegates = self.oop.delegates_to_collaborator(class_fqn)
        matched = len(comp.nodes) == 1 and delegates.matched
        return self._risk_result(class_fqn, "Feature Envy", "LOW", matched, ["feature_envy"],
    ...
    def has_shotgun_surgery(self, class_fqn: str) -> PredicateResult:
        ...
        matched = fin > 10
        return self._risk_result(class_fqn, "Shotgun Surgery", "MEDIUM" if matched else "LOW", matched, ["shotgun_surgery"],
                                 ev_fin, f"High fan-in ({fin}); changing this class may affect many others.")
    ...
    def has_primitive_obsession(self, class_fqn: str) -> PredicateResult:
        # Proxy: parses dicts/json heavily without typing
        json_use = self.core.uses_json(class_fqn)
        return self._risk_result(class_fqn, "Primitive Obsession", "LOW", json_use.matched, ["primitive_obsession"],
                                 set(json_use.evidence_fact_ids), "Heavy use of generic dicts/JSON instead of typed objects.")
    ...
    def has_speculative_generality(self, class_fqn: str) -> PredicateResult:
        ...
        return self._risk_result(class_fqn, "Speculative Generality", "MEDIUM" if matched else "LOW", matched, ["speculative_generality"],
    ...
    def has_circular_dependency(self, class_fqn: str) -> PredicateResult:
        ...
        return self._risk_result(class_fqn, "Circular Dependency", "HIGH" if matched else "LOW", matched, ["circular_dependency"],
                                 cycle.get_evidence_fact_ids(), f"Participates in cycle: {cycle.nodes}")

    def has_excessive_coupling(self, class_fqn: str) -> PredicateResult:
        fout, ev_fout = self.fan_out(class_fqn)
        matched = fout > 10
        return self._risk_result(class_fqn, "Excessive Coupling", "HIGH" if matched else "LOW", matched, ["excessive_coupling"],
                                 ev_fout, f"Fan-out is {fout}.")

    # Maintain existing generic stubs mapped from capabilities.yaml to prevent breakage
    ...
    def has_excessive_abstraction(self, class_fqn: str) -> PredicateResult:
        ...
        matched = depth > 4
        return self._risk_result(class_fqn, "Excessive Abstraction", "MEDIUM" if matched else "LOW", matched, ["deep_inheritance"],
                                 ev, f"Inheritance depth is {depth}.")
    ...
    def has_possibly_unreferenced_symbol(self, class_fqn: str) -> PredicateResult:
        # User warned: fan_in == 0 does not prove dead code. We classify as risk.
        fin, ev = self.fan_in(class_fqn)
        matched = fin == 0
        return self._risk_result(class_fqn, "Unreferenced Symbol", "LOW", matched, ["unreferenced"], ev, "Symbol has 0 internal callers.")
